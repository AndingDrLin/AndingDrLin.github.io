...
> 完整代码见 [`code/06-langgraph/basic_agent.py`](./code/06-langgraph/basic_agent.py)。
...
> 完整代码见 [`code/06-langgraph/complex_workflow.py`](./code/06-langgraph/complex_workflow.py)。
...
> 完整代码见 [`code/06-langgraph/human_in_loop.py`](./code/06-langgraph/human_in_loop.py)，其中包含带 `MemorySaver` checkpoint 的暂停-恢复实现。
...
下一章：[Memory — 让 Agent 拥有记忆](./07-memory.md)。
