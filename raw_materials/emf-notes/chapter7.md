...
本章默认相量时间因子为 $e^{j\omega t}$。如果别的教材采用 $e^{-j\omega t}$，所有含 $j\omega$ 的符号会相反；本课程按 slides 的 $e^{j\omega t}$。
...
slides 强调电场来源有两类：电荷、随时间变化的磁场。可写成
...
|---|---|---|
| 理想绝缘介质 | 无或很小 | 可以存在 |
| 理想导体 | 主要考虑传导电流 | slides 中说明理想导体中不考虑位移电流 |
| 一般介质 | 存在 | 存在 |
...
若介质均匀，即 $\varepsilon,\mu,\sigma$ 为空间常数，则可写成 slides 的 restricted form：
...
边界条件来自 Maxwell 方程的积分形式。取无限薄矩形回路或薄柱面时，面积趋于零，因此 $\partial\vec B/\partial t$ 和 $\partial\vec D/\partial t$ 的面积积分也趋于零。slides 强调：**时变场边界条件的形式与静态场相同**。
...
注意：slides 在 ideal dielectric interface 页面对 $\vec e_n\cdot(\vec D_1-\vec D_2)=0$ 和 $\vec e_n\cdot(\vec B_1-\vec B_2)=0$ 的英文旁注写成 “tangential component”，这与点乘 $\vec e_n\cdot$ 不一致，应理解为**normal component is continuous**。此处按公式和物理意义修正。
...
slides 后面给出 $k=\omega\sqrt{\mu\varepsilon}$，与 $v=\omega/k$ 等价。
...
但 $\vec A$ 方程仍含有 $\mu\varepsilon\,\partial(\nabla\varphi)/\partial t$，所以 slides 说标量位和矢量位仍不能完全分离。
...
slides 总结：在无源区直接求 $\vec E,\vec H$ 和求位函数差不多；在有源区，位函数通常更简单，因为 Lorentz 规范下 $\vec A$ 主要由电流源决定，$\varphi$ 主要由电荷源决定。
...
Homework：slides 列出 7-2, 7-4, 7-7, 7-10, 7-13, 7-17, 7-18, 7-25；题目原文需结合教材或作业册确认。
