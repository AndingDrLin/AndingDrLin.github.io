# 第8章：平面电磁波 (Plane Electromagnetic Waves)
...
| $k_c$ | 有损介质复波数 | slides 记为 $k_c=\beta-j\alpha$ |
...
无损理想介质中 $v_p$ 与频率无关；slides 后面的扩展阅读把“相速度随频率变化”称为色散。
...
对于沿 $+z$ 传播的波，slides 给出：$0<\Delta\phi<\pi$ 为左旋，$-\pi<\Delta\phi<0$ 为右旋。
...
若传播方向不是 $+z$，不要直接套 $x,y$ 相位差口诀。slides 给出一种判断模板：把复振幅写成实部向量 $\vec E_{mr}$ 和虚部向量 $\vec E_{mi}$，再看

$$
\vec k\cdot(\vec E_{mi}\times\vec E_{mr})
$$

在该课件约定下：大于 0 为右旋，小于 0 为左旋，等于 0 为线极化。若还要判断圆/椭圆，再检查 $|\vec E_{mr}|=|\vec E_{mi}|$ 且 $\vec E_{mr}\cdot\vec E_{mi}=0$；满足则为圆极化，否则为椭圆极化。

**补充的基础解释：为了帮助理解而添加。** 这个任意方向判据本质上是在“沿着传播方向看电场端点转动”。固定 $+z$ 口诀只是它的特殊情况。
...
slides 给出的工程理解：高频导线常不用一根很粗的实心线，而用多股细导线或表面镀银/镀金来增加有效导电表面、降低交流电阻；高频发射机线圈可做成空心铜管，因为电流主要在表面流动，内部铜对高频导电贡献很小。这些属于趋肤效应应用，考试重点仍是 $\delta=1/\alpha$ 的计算。
...
- 有损介质中不能直接说 $E$ 和 $H$ 同相。
- 课件中某些生活例子把材料代入通用 $\alpha$ 公式时出现了 $\sigma/(\omega\mu)$ 的排版痕迹；本章统一使用正确无量纲判据 $\sigma/(\omega\varepsilon)$。

### 5.6 色散与群速度（slides 扩展阅读，了解）
...
slides 公式有时直接写 $S=(1+\Gamma)/(1-\Gamma)$，那是在按具体正负讨论；通用计算建议先用 $|\Gamma|$。
...
### 8.3 反射板与 WiFi 信号（slides 应用，了解）

slides 用“易拉罐/金属板增强 WiFi”说明反射和相位叠加：金属板近似理想导体，反射电场相对入射电场反相；但波从天线到反射板再回到天线/目标点还会多走路程，路程差也会带来相位差。
...
低空目标可能出现盲区：直接反射波和地面反射后的波到达雷达时可能近似等幅反相，导致回波减弱。slides 给出斜入射 Fresnel 反射系数作为解释，但斜入射完整推导超出本章正入射主线，考试若未特别说明，掌握“反射会导致相消/相长”即可。
...
17. 任意方向传播时，若 $\vec k\cdot(\vec E_{mi}\times\vec E_{mr})>0$，按 slides 约定是什么旋向？若 $|\vec E_{mr}|=|\vec E_{mi}|$ 且 $\vec E_{mr}\cdot\vec E_{mi}=0$，极化类型是什么？
...
6. $j=e^{j\pi/2}$，所以 $\phi_x=0,\phi_y=+\pi/2$。两分量幅值相等，且
   ...
   \Delta\phi=\phi_y-\phi_x=+\frac\pi2.
   $$
   沿 $+z$ 传播时，slides 约定 $+\pi/2$ 为左旋圆极化，所以答案为
   $$
   \boxed{\text{LHCP}}.
   $$
...
Homework：slides 列出 8-5, 8-6, 8-12, 8-17, 8-22, 8-27；题目原文需结合教材或作业册确认。slides 还包含极化应用、趋肤深度工程应用、煤矿/海水通信、WiFi 反射板、雷达测距与低空盲区等拓展页；本笔记已保留与考试公式直接相关的结论，删除了大部分非考试图片背景。
