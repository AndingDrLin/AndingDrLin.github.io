  ...
  return (
    <div className="quiz-shell">
      <header className="quiz-hero">
        <div>
          <p className="quiz-eyebrow">多学科通用自测平台</p>
      ...
      <div className="quiz-layout">
        <aside className="quiz-panel quiz-controls">
          <h2>筛选与模式</h2>
          <label>
            学科
          ...
          <button className="quiz-btn-secondary" type="button" onClick={() => setShuffleSeed(Date.now())}>
            重新随机
          </button>
          <button className="quiz-btn-danger" type="button" onClick={resetAllRecords}>
            重置当前学科记录
          </button>
        </aside>

        <section className="quiz-panel quiz-card">
          {!currentQuestion ? (
            ...
            <>
              ...
              <div className="quiz-meta-row">
                <span className="quiz-tag quiz-type-tag">题型：{questionTypeLabel(currentQuestion.type)}</span>
                <span className={`quiz-difficulty quiz-difficulty-${currentQuestion.difficulty}`}>{currentQuestion.difficulty}</span>
                {currentQuestion.tags.map((tag) => (
                  <span className="quiz-tag" key={tag}>
                    {tag}
                  </span>
                ))}
                {wrongQuestionIds.includes(currentQuestion.id) && <span className="quiz-tag quiz-warning">错题本</span>}
                {progress.masteredWrongQuestionIds.includes(currentQuestion.id) && <span className="quiz-tag quiz-success">已掌握</span>}
              </div>

              {currentQuestion.type === "multiple" && <p className="quiz-question-hint">本题为多选题，少选、错选、多选均不得分。</p>}
              ...
              <div className="quiz-options">
                {renderableOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      <span className="quiz-option-letter">{currentQuestion.type === "multiple" ? (isSelected ? "✓" : "□") : option.id}</span>
              ...
              {hasSubmitted && (
                <div className={currentRecord?.isCorrect ? "quiz-result quiz-correct-text" : "quiz-result quiz-wrong-text"}>
                  {currentRecord?.isCorrect ? "回答正确" : `回答错误，正确答案：${formatAnswerIds(currentQuestion.answer)}`}
                  <span className="quiz-answer-detail">你的选择：{formatAnswerIds(committedSelection ?? []) || "未选择"}</span>
                </div>
              )}

              <div className="quiz-actions">
                <button className="quiz-btn-secondary" type="button" onClick={previousQuestion} disabled={currentIndex === 0}>
                  上一题
                </button>
                {!shouldReveal && (
                  <button className="quiz-btn-secondary" type="button" onClick={() => setShowExplanation(true)}>
                    查看解析
                  </button>
                )}
                {hasSubmitted && (
                  <button className="quiz-btn-secondary" type="button" onClick={startRetry}>
                    重新作答
                  </button>
                )}
                <button className="quiz-btn-primary" type="button" onClick={submitAnswer} disabled={draftSelection.length === 0 || (hasSubmitted && !isRetrying)}>
                  提交答案
                </button>
                <button className="quiz-btn-primary quiz-ghost" type="button" onClick={nextQuestion} disabled={currentIndex >= filteredQuestions.length - 1}>
                  下一题
                </button>
              </div>

              {shouldReveal && (
                <article className="quiz-explanation">
                  <h3>解析</h3>
                  <p>
        ...
        <aside className="quiz-panel quiz-stats">
          <h2>统计</h2>
          ...
          <h3>标签正确率</h3>
          <div className="quiz-mini-list scroll">
