---
title: "第6章 静磁场"
description: "电磁场与波第6章：静磁场基本方程、Biot-Savart定律、安培环路定律、磁介质、边界条件、电感与磁能。"
date: 2026-05-18
...
| $\bm e_n$ | 从介质2指向介质1的界面单位法向量（本章边界条件统一采用该方向） | 1 |
...
矢量场只给旋度还不唯一，静磁场常用 Coulomb gauge：
...
安培力定律说明：两个载流回路之间会相互作用。若回路 $C_1$ 中电流为 $I_1$，回路 $C_2$ 中电流为 $I_2$，从 $C_1$ 上电流元指向 $C_2$ 上电流元的矢量为 $\bm R_{12}$，则 $C_1$ 对 $C_2$ 的力可写成
...
**直观理解：** 静电学中“电荷 $\to \bm E \to$ 作用力”；静磁场中对应为“电流 $\to \bm B \to$ 对其他电流产生安培力”。所以本章计算 $\bm B$，不是为了单独看一个抽象矢量，而是为了后续算磁力、磁能和电感。
...
其中 $\theta_1,\theta_2$ 按本节图示从导线方向（$+z$ 方向）量到“源点到场点连线”的方向：图中下端对应 $\theta_1$，上端对应 $\theta_2$，所以这里写成 $\cos\theta_1-\cos\theta_2$。
...
内导体半径 $a$，外导体内半径 $b$、外半径 $c$。内导体电流 $+I$，外导体均匀回流 $-I$。分段结果为：
...
本节统一采用下面的法向约定：$\bm e_n$ 从介质2指向介质1。因此公式都写成“1 减 2”：
...
等价地，若只看图中由 $\Delta\bm l=\bm N\times\bm e_n\Delta l$ 定义的切向方向分量：
...
磁矢位边界条件通常写为：严格说，物理上最直接的是 $\bm B$、$\bm H$ 的边界条件；在采用同一规范并要求 $\bm A$ 不出现不必要奇异跳变时，可写为
...
内导体半径 $a$，外导体内半径 $b$，若只考虑内导体内部和两导体之间的磁链：
...
若外导体也有有限厚度并考虑其内部磁链，要再加外导体内自感项；完整计算还需加入该项。
...
注意第一项必须是 $I_1^2$。第一项必须含 $I_1^2$；从量纲也能看出 $L_1I_1$ 不是能量。
...
期末考试 Q2 专门考第6章，10 分大题。5 年真题（2022–2025 + Mock）反复出现以下题型。

### 12.1 考试高频题型一览

| 题型 | 出现频率 | 典型问法 |
|---|---|---|
| 安培环路定律求 B/H | ★★★★★ | 同轴线、螺线管、无限大电流片 |
| 磁化电流 J_M, J_sM | ★★★★ | 给 M 求磁化电流；给 μ 求 B→M→J |
| 自感计算 | ★★★★★ | 同轴线单位长度自感、螺线管自感 |
| 互感计算 | ★★★★ | 长直导线与矩形线圈、两螺线管 |
| 磁场能量 | ★★★ | 用场分布或电感求磁能 |
| B/H 边界条件 | ★★★★ | 两磁介质界面、法向切向分量 |
| Lorentz 力/安培力 | ★★★ | 矩形线圈在磁场中的力和力矩 |
| 磁矢位 A | ★★★ | 面电流/线电流产生的 A，由 A 求 B |

### 12.2 做题套路：B-H-M-JM 全套求解

**这是 Q2 最常见的大计算题模板。** 给一个含磁介质的结构，要求从 H 一路算到 JM。

**固定套路：**

1. **先求 H**：用安培环路定律 $\oint \bm H\cdot d\bm l=I_{\text{free,enc}}$（H 只由自由电流决定，与介质无关）
2. **再求 B**：用本构关系 $\bm B=\mu\bm H$（各区域 μ 不同，B 就不同）
3. **求 M**：$\bm M=\bm B/\mu_0-\bm H$ 或 $\bm M=\chi_m\bm H$
4. **求 JM**：$\bm J_M=\nabla\times\bm M$（体磁化电流）、$\bm J_{sM}=\bm M\times\bm e_n$（面磁化电流）

**真题示例（2022/2025 综合）：** 磁性圆柱（半径 a，磁导率 μ）内有轴线线电流 I，外部为空气。

$$\bm H=\bm e_\phi\frac{I}{2\pi\rho} \quad \text{（内外形式相同！）}$$

$$\bm B=\begin{cases}\bm e_\phi\frac{\mu I}{2\pi\rho}, & \rho<a \\ \bm e_\phi\frac{\mu_0 I}{2\pi\rho}, & \rho>a\end{cases}$$

$$\bm M=\begin{cases}\bm e_\phi\frac{(\mu-\mu_0)I}{\mu_0\cdot 2\pi\rho}, & \rho<a \\ 0, & \rho>a\end{cases}$$

**关键陷阱：** H 由自由电流决定，内外形式相同（$I/2\pi\rho$）；B 要乘各区域的 μ，所以内外不同。考试中这是最常见的丢分点。
...
$$L=\frac{\Psi}{I}, \quad \Psi=\int_S \bm B\cdot d\bm S$$

**方法二：磁能法**（适合复杂分区）

$$L=\frac{2W_m}{I^2}, \quad W_m=\frac{1}{2}\int_V \mu H^2\,dV$$

**同轴线自感（必背）：**

$$L'=\frac{\mu_0}{8\pi}+\frac{\mu_0}{2\pi}\ln\frac{b}{a}$$

第一项是内自感（导体内部），第二项是外自感（两导体之间）。如果题目说"忽略导体内部磁链"，只保留第二项。

**螺线管自感（必背）：**

$$L=\mu_0 n^2 l \pi a^2 = \mu_0 \frac{N^2}{l} \pi a^2$$

其中 n 是单位长度匝数，N 是总匝数，l 是长度，a 是半径。

### 12.4 做题套路：互感计算

**考试只考一种模型：长直导线 + 矩形线圈。**

$$M=\frac{\mu_0 a}{2\pi}\ln\frac{D+b}{D}$$

其中 a 是线圈高度，b 是宽度，D 是近边距导线距离。

**步骤：**
1. 用安培定律求长直导线的 B：$B=\mu_0 I/(2\pi r)$
2. 对线圈面积积分求磁链：$\Psi=\int_D^{D+b}\frac{\mu_0 I}{2\pi r}a\,dr$
3. 除以电流得互感：$M=\Psi/I$

**2024 试卷还考了螺线管互感：** $M=n_1 n_2 \mu_0 V$（两螺线管共轴套在一起时）。

### 12.5 做题套路：磁场边界条件

**两磁介质界面（无自由面电流）：**

$$B_{1n}=B_{2n} \quad \text{（法向 B 连续）}$$
$$H_{1t}=H_{2t} \quad \text{（切向 H 连续）}$$
...
1. 分解 H₁ 为法向和切向分量
2. 切向直接连续（或用面电流修正）
3. 法向用 $B_{1n}=B_{2n}$ 即 $\mu_1 H_{1n}=\mu_2 H_{2n}$，注意要乘 μ！

### 12.6 做题套路：安培力与 Lorentz 力

**线电流在磁场中的力：**

$$\bm F=I\oint d\bm l\times\bm B$$

**矩形线圈在均匀磁场中的力矩：**

$$\bm T=\bm m\times\bm B, \quad \bm m=NIS\bm e_n$$

**2025 考了矩形线圈各段受力：** 线圈在非均匀磁场 $\bm H=\bm a_x 3x^2$ 中，需要对每一段分别计算 $\bm F=I\int d\bm l\times\bm B$。

### 12.7 真题逐题索引

| 年份 | 题号 | 考点 |
|---|---|---|
| 2022 | Q3 | Lorentz 力、矩形线圈力矩 |
| 2022 | Q4 | 磁矢位定义、面电流产生 B |
| 2022 | Q9(c,d) | 感应电动势、同轴线自感 |
| 2023 | Q3 | 互感（螺线管）、B 边界条件、螺线管自感 |
| 2024 | Q4 | 自感概念、同轴线自感、环形线圈自感、H 与 B 关系 |
| 2024 | Q5 | 法拉第定律、感应电动势、边界条件 |
| 2025 | Q4 | 磁矢位、磁偶极子、面电流 B、同轴线自感 |
| 2025 | Q5 | Lorentz 力、矩形线圈受力、H/B 边界条件 |
| Mock | Q2 | 同轴线 H 分区、磁能密度、单位长度自感 |
...
| 有限长导线角度乱背 | 不看图直接套 $\cos\theta_1-\cos\theta_2$ | 本节的角度从导线方向量起；若从垂线量起常写 $\sin\alpha_1+\sin\alpha_2$ |
...
- 第6章的电感和磁能是理解线圈、变压器、电磁炮/电磁弹射等应用的基础。
