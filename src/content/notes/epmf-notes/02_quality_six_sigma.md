...
$$
UCL = \bar{x} + k \sigma, \qquad LCL = \bar{x} - k \sigma
$$

其中 $k$ 通常取 3。PPT 用 normal curve 解释：1σ 大约覆盖 68.26%，2σ 覆盖 95.44%，3σ 覆盖 99.73%。如果题目明确说 2σ，就用 2；如果没说，考试通常按 3σ。

**口径提醒：** $\sigma$ 是总体标准差，$s$ 是样本标准差（其实用 $s$ 会更合适，但是 PPT 中给出的是 $\sigma$）；如果考试题直接给出 $\sigma$、给出常数 $A$，或老师指定总体标准差，就按题目给定口径算。通常数值会略有差异，但判断逻辑不变。
...
**Step 2：按 $\sigma$ 口径算标准差。**

如果手算，做法是先算每个 $(x_i-\bar{x})^2$，再求和、除以 $n$、开方。例如第 1 个数据的平方差约为 $(98.50-98.415)^2=0.0072$，第 6 个数据的平方差约为 $(94.32-98.415)^2=16.77$。考试中通常可以用计算器直接求 standard deviation。

$$
\sigma \approx 1.164
$$

这里按 PPT 的 $\sigma$ 口径使用总体标准差形式，即分母用 $n$。

**Step 3：算 3σ 控制限。**

$$
UCL = 98.415 + 3\sigma = 98.415 + 3 \times 1.164 \approx 101.91
$$

$$
LCL = 98.415 - 3\sigma = 98.415 - 3 \times 1.164 \approx 94.92
$$
...
$$
UCL \approx 100.74, \qquad LCL \approx 96.09
$$
...
$$
\bar{L} \approx k\left[(\bar{y}-m)^2+\sigma^2\right]
$$

其中 $(\bar{y}-m)^2$ 表示过程中心偏离 target 的损失，$\sigma^2$ 表示总体方差带来的损失。这就是电话线例子想说明的点：只减少 defect fraction 不够，如果均值偏离 target 或方差很大，客户仍然会感到质量差。
...
某过程 5 次测量值为 `10, 11, 9, 10, 15`，平均值 $\bar{x}=11$，按总体标准差口径 $\sigma \approx 2.10$。如果用 2σ 控制限，UCL/LCL 是多少？15 这个点是否超限？

$$
UCL=11+2\times2.10=15.20
$$

$$
LCL=11-2\times2.10=6.80
$$
...
| 年份 | 题号 | 考查形式 | 考查内容 |
|---|---|---|---|
| 2022 | Q2 | MCQ | 控制图基本概念判断（UCL/LCL 稳定性） |
...
| 2023 | Q18 | MCQ | 7QC 工具识别（给描述选工具） |
| 2023 | Q14 | 计算 | Taguchi 损失平均值计算 |
| 2023 | Q15 | 计算 | 控制图计算（CL/UCL/LCL） |
| 2023 | Q16 | 计算 | Cp/Cpk 计算与过程能力评价 |
| 2024 | Q9 | MCQ | 控制图用途判断（稳定性 vs 其他） |
| 2024 | Q7 | 计算 | 控制图 + Cpk 综合计算 |
| 2024 | Q8 | 计算 | Taguchi loss 计算（给定 k 和观测值） |
| 2025 | Q4(i) | 简答 | Taguchi 损失函数原理 + 三类损失 |
| 2025 | Q4(ii) | 计算 | 控制图 CL/UCL/LCL 计算 + 稳定性分析 |
| 2025 | Q4(iii) | 计算 | Cp/Cpk 计算 + 过程能力解释 |
