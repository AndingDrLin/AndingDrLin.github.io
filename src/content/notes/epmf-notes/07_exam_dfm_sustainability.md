...
| 类型 | 英文 | 适用场景 | 损失函数 | 目标值 |
|------|------|----------|----------|--------|
| 望目特性 | Nominal-the-best (NTB) | 尺寸、电压等有明确目标值 | $L(y) = k(y - m)^2$ | $m$ = 标称值 |
| 望小特性 | Smaller-the-better (STB) | 噪音、磨损、误差 | $L(y) = k y^2$ | $m = 0$ |
| 望大特性 | Larger-the-better (LTB) | 强度、寿命等 | $L(y) = k / y^2$ | 越大越好 |

**NTB** 偏离目标越大损失越大，是最常考计算的类型。**STB** 的理想值是零（如零排放、零缺陷），公式中 $m=0$，所以 $L(y-m)^2$ 简化为 $ky^2$。**LTB** 的理想值趋向无穷大（如材料强度越高越好），用倒数 $k/y^2$ 表示：$y$ 越大，损失越小。

**考试只考 NTB 的计算**（Nominal-the-best），就是 $L(y) = k(y - m)^2$ 这个公式。STB 和 LTB 多在概念识别题中出现。
