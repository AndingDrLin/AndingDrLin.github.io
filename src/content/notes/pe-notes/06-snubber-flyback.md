...
这是反激题的高频考点。开关关断期间，输出二极管导通，加在二极管两端的反向电压来自两个部分：

1. 输出电压 $V_o$——二极管截止时，输出电容上的电压直接加在二极管上。
2. 输入电压折算到次级——开关导通时，$V_{in}$ 加在初级上，通过变压器耦合到次级为 $V_{in}\times N_s/N_p$，方向与 $V_o$ 叠加。
...
> **推导为什么是相加而非相减？** 从同名端看：开关导通期间，初级电压 $V_{in}$ 上正下负（同名端为正），次级感应电压方向为上负下正（同名端也为正但次级二极管接法使此电压反向加在二极管上）。所以次级二极管两端的反向电压 = 输出电压 + 感应电压，两者同向叠加。
...
- 反激二极管 PIV 公式是 $PIV=V_o+(N_s/N_p)\cdot V_{in}$，两个电压同向叠加，不要写成相减。开关关断时次级感应电压和输出电压都正向加在二极管反向端。
